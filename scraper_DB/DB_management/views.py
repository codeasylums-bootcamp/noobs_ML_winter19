...
def about(request):
    ...
    #     "flights" : flights
    # }
    return render(request,'DB_management/about.htm')
...
class FlightDataView(viewsets.ModelViewSet):
    ...
    def update_flightdata(self, request, *args, **kwargs):
    ...
    def delete_flight_data(self, request,*args, **kwargs):
        delete_flight = FlightData.objects.filter()
        filter_kwargs = {self.lookup_field : self.kwargs[self.lookup_field]}
        serializer = FlightDataSerializer(data = request.data)
        if serializer.is_valid():
            validated_data = serializer.validated_data
